...
public class MeatTreeCycleOverlay extends OverlayPanel {
	...
	public Dimension render(Graphics2D graphics) {
		...
		if (this.config.meatTreeChopCycle() == CoxAdditionsConfig.meatTreeChopCycleMode.INFOBOX && this.client.getVar(Varbits.IN_RAID) == 1 && this.plugin.startedChopping && this.plugin.meatTreeAlive) {
			this.panelComponent.setPreferredSize(new Dimension(graphics.getFontMetrics().stringWidth("Tick:   ") + 15, 0));
			this.panelComponent.getChildren().add(LineComponent.builder().left("Tick: ").right(String.valueOf(this.plugin.ticksToChop)).build());
		}
