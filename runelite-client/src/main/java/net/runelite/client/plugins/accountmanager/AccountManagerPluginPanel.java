...
public class AccountManagerPluginPanel extends PluginPanel
{
	...
	private void setupLayout()
	{
		...
		add(parent);
		parent.setToolTipText("<html><font color=#ff6961>Do Not Use Commas in username or description you will break things!");
		...
		var descriptionLabelConstraints = c.clone();
		parent.add(descriptionLabel, descriptionLabelConstraints);
		c.gridy++;

		JTextField description = new JTextField();
		var descriptionFieldConstraints = c.clone();
		parent.add(description, descriptionFieldConstraints);
		c.gridy++;
		...
		addAccountButton.addMouseListener(new MouseAdapter()
		{
			...
			{
				addAccountButtonClicked(parent, username, password, description);
			}
		});
	...
	private void addAccountButtonClicked(JComponent parent, JTextField username, JPasswordField password, JTextField description)
	{
		boolean result = add(username.getText(), password.getPassword(), description.getText());
		if (!result)
		{
	...
	private JComponent createItem(boolean isDefault, OsrsCredential credential)
	{
		...
		String userDetails = credential.getCredential().Username;
		...
		String usernameLabelText = username.substring(0, Math.min(10, username.length()));
		...
		JLabel descriptionLabel = new JLabel("Description");
		var descriptionLabelConstraints = c.clone();
		sectionContents.add(descriptionLabel, descriptionLabelConstraints);
		c.gridy++;

		JTextField descriptionField;
		if(parts.length == 1) {
			descriptionField = new JTextField(parts[0]);
		} else {
			descriptionField = new JTextField(parts[1]);
		}
		var descriptionFieldConstraints = c.clone();
		sectionContents.add(descriptionField, descriptionFieldConstraints);
		c.gridy++;
		...
		updateButton.addMouseListener(new MouseAdapter()
		{
			...
			public void mouseClicked(MouseEvent e)
			{
				Credential _credential = new Credential(usernameField.getText() + "," + descriptionField.getText(), new String(passwordField.getPassword()));
	...
	private boolean add(String username, char[] password, String description)
	{
		int index = firstFreeIndex(accountIndices);
		Credential credential = new Credential(username + "," + description, new String(password));
		String key = OSRS_KEY_PREFIX + index;
	...
	private void setLogin(final OsrsCredential credential)
	{
		...
		{
			String user_desc = credential.getCredential().Username;
			String[] parts = user_desc.split(",");
