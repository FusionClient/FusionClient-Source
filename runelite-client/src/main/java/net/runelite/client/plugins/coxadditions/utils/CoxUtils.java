...
public class CoxUtils {
	...
	private static final char[] SORTS;
	private static final String[] NAMES;

	public static int getroom_type(int zonecode) {
		switch(zonecode & 67059680) {
		case 6689792:
		case 6689824:
		case 6755360:
			return 1;
	...
	public static char getroom_sort(int roomtype) {
		return roomtype >= 0 && roomtype < 16 ? SORTS[roomtype] : '?';
	}

	public static String getroom_name(int roomtype) {
		return roomtype >= 0 && roomtype < 16 ? NAMES[roomtype] : "Unknown";
	}

	public static int getroom_winding(int zonecode) {
		return (zonecode >> 16 & 255) - 103 & 3;
	}
	...
	public static int getroom_exitside(int zonecode) {
		return getroom_winding(zonecode) + getroom_rot(zonecode) & 3;
	}

	static {
		SORTS = new char[]{'*', '*', 'S', 'F', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'P', 'P', 'P', 'P'};
		NAMES = new String[]{"Floor start", "Floor end", "Scavengers", "Farming", "Shamans", "Vasa", "Vanguards", "Mystics", "Tekton", "Muttadiles", "Guardians", "Vespula", "Ice demon", "Thieving", "Tightrope", "Crabs"};
	}
}
